...
4. Tell the user the file path, the revision number, and that they need to fill in the `upgrade` / `downgrade` bodies (or ask Codex to do it based on the model diff). Remind them migrations apply automatically on `just up` via the entrypoint — no manual `alembic upgrade head`.
