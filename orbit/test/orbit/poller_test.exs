defmodule Orbit.PollerTest do
  ...
  test "an unreachable box is an error, not an empty success" do
    # DELIBERATE DIVERGENCE from python: the vendor clients swallow per-endpoint
    # failures, so a dead box yields zero sections instead of raising. python's
    # _poll_instance stamps last_success_at anyway and writes cpu.total=0 — the
    # box never goes offline, it flatlines at 0%. Zero sections must be an error.
    Req.Test.stub(Orbit.Poller.OpnsenseClient, fn conn ->
      Plug.Conn.send_resp(conn, 502, "bad gateway")
    end)

    assert Poller.poll_instance(direct_instance(900_002), persist: false) ==
             {:error, :no_data}
  end
end
