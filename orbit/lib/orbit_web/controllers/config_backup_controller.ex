defmodule OrbitWeb.ConfigBackupController do
  ...
  def diff(conn, %{"instance_id" => raw_id, "backup_id" => raw_bid}) do
    user = conn.assigns.current_user
  ...
  defp diff_response(conn, {:ok, text, truncated}) do
    body = if truncated, do: text <> "\n\n(diff truncated)", else: text
    conn |> put_resp_content_type("text/plain") |> send_resp(200, body)
  end

  defp diff_response(conn, :no_previous) do
    conn |> put_resp_content_type("text/plain") |> send_resp(200, "(no previous version to diff)")
  end

  defp diff_response(conn, :error), do: conn |> put_status(404) |> json(%{detail: "not found"})
end
