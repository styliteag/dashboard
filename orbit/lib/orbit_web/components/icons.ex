defmodule OrbitWeb.Components.Icons do
  @moduledoc """
  Vendored lucide icon catalog (24x24 stroke paths, no icon dependency) —
  the one place icon path data lives. Used qualified (`<Icons.icon
  name={:bell} />`, aliased in html_helpers) because CoreComponents already
  owns the unqualified `icon/1` for hero-* names.

  Keys are semantic: nav page keys (:instances, :hub, …, matching the old
  Layout.tsx mapping) plus content glyphs (:bell_off, :power, :bot, …).
  Unknown keys render nothing, so a caller with a stale key never crashes
  the page.
  """
  ...
  # -- nav page keys (Layout.tsx parity; vpn=lock and audit=history are
  # deliberate upgrades — see the nav-icons commit) ------------------------

  defp paths(:instances),
    do:
  ...
  defp paths(:brand), do: ~s(<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>)
  ...
  defp paths(:bell_off),
    do:
      ~s(<path d="M8.7 3A6 6 0 0 1 18 8a21.3 21.3 0 0 0 .6 5"/><path d="M17 17H3s3-2 3-9a4.67 4.67 0 0 1 .3-1.7"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/><line x1="2" x2="22" y1="2" y2="22"/>)

  defp paths(:eye_off),
    do:
      ~s(<path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"/><path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68"/><path d="M6.61 6.61A13.526 13.526 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61"/><line x1="2" x2="22" y1="2" y2="22"/>)

  defp paths(:power), do: ~s(<path d="M12 2v10"/><path d="M18.4 6.6a9 9 0 1 1-12.77.04"/>)

  defp paths(:bot),
    do:
      ~s(<path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/>)

  defp paths(:list_checks),
    do:
      ~s(<path d="m3 17 2 2 4-4"/><path d="m3 7 2 2 4-4"/><path d="M13 6h8"/><path d="M13 12h8"/><path d="M13 18h8"/>)
  ...
  defp paths(:sliders),
    do:
      ~s(<line x1="21" x2="14" y1="4" y2="4"/><line x1="10" x2="3" y1="4" y2="4"/><line x1="21" x2="12" y1="12" y2="12"/><line x1="8" x2="3" y1="12" y2="12"/><line x1="21" x2="16" y1="20" y2="20"/><line x1="12" x2="3" y1="20" y2="20"/><line x1="14" x2="14" y1="2" y2="6"/><line x1="8" x2="8" y1="10" y2="14"/><line x1="16" x2="16" y1="18" y2="22"/>)

  defp paths(:message_square),
    do: ~s(<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>)

  defp paths(:send), do: ~s(<path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/>)
  ...
  defp paths(:terminal),
    do: ~s(<polyline points="4 17 10 11 4 5"/><line x1="12" x2="20" y1="19" y2="19"/>)
  ...
  # lucide: scan-line — packet capture.
  defp paths(:capture),
    do:
      ~s(<path d="M3 7V5a2 2 0 0 1 2-2h2"/><path d="M17 3h2a2 2 0 0 1 2 2v2"/><path d="M21 17v2a2 2 0 0 1-2 2h-2"/><path d="M7 21H5a2 2 0 0 1-2-2v-2"/><path d="M7 12h10"/>)

  # lucide: chart-line — the tunnel uptime graph dialog.
  defp paths(:chart),
    do: ~s(<path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="m19 9-5 5-4-4-3 3"/>)

  # lucide: wrench — maintenance marker (replaced a 🛠️ emoji, which renders
  # differently per OS and ignores the surrounding text colour).
  defp paths(:maintenance),
    do:
