...
Split from the firewall line (orbit_agent.py, OPNsense/pfSense): the dashboard
serves this file to device_type=linux instances; on the box it still lands at
/usr/local/orbit-agent/orbit_agent.py, so supervisor and systemd unit are
unchanged. Phase 1 of the split: full copy with its own identity — the
firewall-only code (relay, GUI login/proxy, firmware, tunnels) is stripped in
a later phase. The vendored Checkmk agent does the collecting; this file
transports its raw output (§25/DR-10).
