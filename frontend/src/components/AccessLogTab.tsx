...
export default function AccessLogTab() {
  const [kinds, setKinds] = useState<string[]>(["auth", "denial", "request"]);
  ...
  const timeline = useQuery({
    queryKey: ["access-timeline", kindsParam],
    queryFn: () =>
      api.get<TimelinePage>(`/api/access-log/timeline?kinds=${kindsParam}&limit=${PAGE_LIMIT}`),
    refetchInterval: 30_000,
  });
  ...
  const loadOlder = async () => {
    ...
    try {
      const page = await api.get<TimelinePage>(
        `/api/access-log/timeline?kinds=${kindsParam}&limit=${PAGE_LIMIT}&before=${encodeURIComponent(before)}`,
      );
  ...
  return (
    <div>
      {summary.isError && (
        <p className="mt-4 text-sm text-red-400">{apiErrorText(summary.error, "Failed to load access summary")}</p>
      )}
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={<Users className="h-4 w-4" />} label="Online now">
          ...
          <ul className="space-y-1">
            {s?.online.map((o) => (
              <li key={`${o.user_id}-${o.login_at}`} className="flex items-baseline justify-between gap-2 text-sm">
        ...
        <StatCard icon={<LogIn className="h-4 w-4" />} label="Logins (24h)">
          <div className="flex items-baseline gap-4">
            <span className="text-2xl font-semibold text-emerald-400">{s?.logins_ok_24h ?? "…"}</span>
