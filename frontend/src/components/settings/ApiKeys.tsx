...
export interface ApiKeysProps {
  ...
  defaultName?: string;
  /** Intro paragraph shown above the controls. */
  intro?: ReactNode;
  /** Label shown in the "just created" success box. */
  createdLabel?: string;
  /** Content for the usage example block (usually an env or config snippet). */
  usageExample?: (key: string) => ReactNode;
  /** Optional additional help text under the header. */
  help?: ReactNode;
  /** If set, keys are tagged with this purpose on creation and the list is filtered
      to matching keys (plus legacy keys with no purpose). */
  purpose?: string;
}

const DEFAULT_INTRO = (
  <>
    Read-only service-account keys (Bearer <code className="text-slate-300">orbit_…</code>, rejected
    on non-GET). Keys created here are <strong>re-viewable</strong>: the token is kept encrypted so
    you can copy it again later. Revoking drops that copy.
  </>
);
...
export default function ApiKeys({
  ...
  const matchesPurpose = (k: ApiKey) => !purpose || !k.purpose || k.purpose === purpose;
  const active = keys.filter((k) => !k.revoked_at && matchesPurpose(k));
  const revoked = keys.filter((k) => k.revoked_at && matchesPurpose(k));
    ...
    <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
      ...
      <div className="mt-4 flex items-center gap-2">
        <input
          ...
          placeholder={`Key name (e.g. ${defaultName})`}
        ...
        <tbody>
          ...
          {active.map((k) => (
            <tr key={k.id} className="border-t border-slate-800">
              ...
              <td className="py-2">
                {k.purpose ? (
                  <span className="rounded bg-slate-800 px-1.5 py-0.5 text-[10px] font-mono text-emerald-300">
                    {k.purpose}
                  </span>
                ) : (
              ...
              {revoked.map((k) => (
                <tr key={k.id} className="border-t border-slate-800 text-slate-500">
                  ...
                  <td className="py-2 text-xs">
                    {k.purpose ? (
                      <span className="rounded bg-slate-800 px-1 py-0.5 font-mono text-[10px] text-emerald-300/70">
                        {k.purpose}
                      </span>
                    ) : null}
                  </td>
