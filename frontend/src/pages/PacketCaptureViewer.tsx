...
export default function PacketCaptureViewer() {
  ...
  const [liveMeta, setLiveMeta] = useState<any>({});
  ...
  const parseIncomingPcap = () => {
    ...
    const newPkts: any[] = [];
    let pktIdx = livePackets.length + 1;
  ...
  const summarizePacket = (frame: Uint8Array, ts: number, idx: number) => {
    if (frame.length < 14) {
      return { idx, ts: ts.toFixed(3), src: "", dst: "", proto: "RAW", len: frame.length, info: "", hex: toHex(frame.slice(0, 64)) };
    }
    ...
    if (ethType === 0x86dd) {
      return { idx, ts: ts.toFixed(3), src: "", dst: "", proto: "IPv6", len: frame.length, info: "", hex: toHex(frame.slice(0, 64)) };
    }
    return { idx, ts: ts.toFixed(3), src: "", dst: "", proto: "ETH", len: frame.length, info: `0x${ethType.toString(16)}`, hex: toHex(frame.slice(0, 64)) };
  };

  const parseIPv4 = (ip: Uint8Array, ts: number, idx: number, wireLen: number) => {
    if (ip.length < 20) return { idx, ts: ts.toFixed(3), src: "", dst: "", proto: "IP", len: wireLen, info: "", hex: toHex(ip.slice(0, 64)) };
    const ihl = (ip[0] & 0x0f) * 4;
    const proto = ip[9];
    const src = `${ip[12]}.${ip[13]}.${ip[14]}.${ip[15]}`;
    const dst = `${ip[16]}.${ip[17]}.${ip[18]}.${ip[19]}`;
    const l4 = ip.slice(ihl);
    if (proto === 6 && l4.length >= 4) { // TCP
      ...
      return { idx, ts: ts.toFixed(3), src: `${src}:${sport}`, dst: `${dst}:${dport}`, proto: "TCP", len: wireLen, info, hex: toHex(ip.slice(0, 96)) };
    }
    if (proto === 17 && l4.length >= 4) { // UDP
      const sport = (l4[0] << 8) | l4[1];
      const dport = (l4[2] << 8) | l4[3];
      let info = `${sport}→${dport}`;
      if (dport === 53 || sport === 53) info += " DNS/mDNS";
      return { idx, ts: ts.toFixed(3), src: `${src}:${sport}`, dst: `${dst}:${dport}`, proto: "UDP", len: wireLen, info, hex: toHex(ip.slice(0, 80)) };
    }
    return { idx, ts: ts.toFixed(3), src, dst, proto: `IP/${proto}`, len: wireLen, info: "", hex: toHex(ip.slice(0, 64)) };
  };

  const toHex = (b: Uint8Array) => Array.from(b).map(x => x.toString(16).padStart(2, "0")).join(" ");

  // Static (one-shot) mode
  const { data: staticData, isLoading, error } = useQuery<CaptureData>({
    queryKey: ["capture-view", capId],
    queryFn: () => api.get(`/api/captures/${capId}/packets`),
  ...
  const meta = isLive ? liveMeta : (staticData?.meta || {});
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (wsRef.current) {
        try { if (isLive) wsRef.current.send(JSON.stringify({ type: "stop" })); } catch {}
        wsRef.current.close();
      }
      wsRef.current = null;
    };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl font-semibold flex items-center gap-2">
              ...
              {isLive && !isStopped && <span className="text-emerald-400 text-sm animate-pulse">● LIVE</span>}
          ...
          <div className="flex gap-2">
            {isLive && (
              <>
                ...
                <button onClick={downloadLive} className="inline-flex items-center gap-2 rounded bg-emerald-600 px-3 py-1.5 text-sm hover:bg-emerald-500">
                  <Download className="h-4 w-4" /> Save Buffer as PCAP
                </button>
              </>
            )}
            {!isLive && (
              <a href={downloadUrl} className="inline-flex items-center gap-2 rounded bg-emerald-600 px-3 py-1.5 text-sm hover:bg-emerald-500">
        ...
        {meta && (
          <div className="mb-3 text-xs text-slate-400 flex flex-wrap gap-x-4">
            <span>iface: <span className="font-mono text-slate-300">{meta.interface || liveIface}</span></span>
            {(meta.filter || liveFilter) && <span>filter: <span className="font-mono text-slate-300">{meta.filter || liveFilter}</span></span>}
            <span>bytes received: <span className="font-mono text-emerald-300">{bytesReceived}</span></span>
            ...
            {isLive && captureStarted && !isStopped && <span className="text-emerald-400"> (capture started on box)</span>}
            {isLive && isStopped && captureStarted && <span className="text-slate-400"> (stopped)</span>}
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          />
          {isLive && !isStopped && <span className="text-xs text-emerald-400">Live — new packets append automatically</span>}
          {isLive && isStopped && <span className="text-xs text-slate-400">Stream stopped — packets no longer append</span>}
        </div>

        {isLoading && <div className="p-4 text-sm text-slate-400">Loading packets…</div>}
        {error && <div className="p-4 text-sm text-red-400">Failed to load: {(error as any).message}</div>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
              ...
              <tbody className="divide-y divide-slate-800">
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-3 py-4 text-slate-500">
                      {captureError ? `Error from agent: ${captureError}` :
                       isStopped 
                ...
                {filtered.map((p: any, i: number) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-0.5 font-mono text-xs">{typeof p.ts === "number" ? p.ts.toFixed(3) : p.ts}</td>
                    <td className="px-2 py-0.5 font-mono text-xs text-emerald-300 truncate" title={p.src}>{p.src}</td>
                    <td className="px-2 py-0.5 font-mono text-xs text-sky-300 truncate" title={p.dst}>{p.dst}</td>
                    <td className="px-2 py-0.5"><span className="rounded bg-slate-800 px-1 text-[10px]">{p.proto}</span></td>
          ...
          <div className="lg:col-span-2 border border-slate-800 rounded-xl bg-slate-900/60 p-3 min-h-[320px]">
            {!selected && <div className="text-sm text-slate-400">Select a packet to inspect its hex.</div>}
            {selected && (
              <>
                <div className="text-sm mb-2">
                  <span className="text-emerald-400">#{selected.idx}</span> {selected.src} → {selected.dst} <span className="text-slate-400">({selected.proto})</span>
                </div>
