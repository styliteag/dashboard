...
- **Agent self-update refuses downgrades (anti-rollback)** — the agent verified an update's Ed25519 signature but not its version, so a compromised dashboard could replay an older — still validly signed — agent build to reintroduce a fixed vulnerability. The agent now rejects any pushed build whose embedded `__version__` is not strictly newer than the running one. The check reads the version from the signature-covered code (not the unsigned push parameter), so it can't be forged. (Agent `__version__` → 1.6.3.)
