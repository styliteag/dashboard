...
- Linux agent version is now 4.4.9, matching the dashboard release.
...
- The Firmware page can start bulk updates again (parity with the old
  ...
  for agent-mode boxes with a resolved target — "Major version upgrade"
  buttons behind an "N selected:" bar (the upgrade button shows "x of N"
  and a tooltip when it covers fewer boxes than are selected, and
  hovering either button highlights exactly the rows it will act on).
  Filtering the list down also narrows what a bulk action fires on;
  firmware-locked and out-of-scope instances are never acted on.

- "Series upgrade" is now called "Major version upgrade" everywhere
  (firmware page, instance page, bulk menu, refusal messages) — operators
  asked what a "series" is; the vendors' jargon explained nothing.
...
- Connectivity page: monitor rows now expand like the VPN page's tunnels —
  the chevron opens an RTT history chart for that ping monitor
  (range-switchable 1h/6h/24h/7d), chart colour tracking the monitor's
